...
public class ActivateDeactivate : UserControllerSetup
{
    ...
    public async Task ActivateDeactivate_ValidationRoles_ReturnStatusCodeOk(string role)
    {
        const int statusForbidden = 403;
        const int statusOk = 200;

        var methodInfo = typeof(UserController).GetMethod("ActivateDeactivate");
        ...
        UserCommandService
            .Setup(s => s.ActivateOrDeactivateAsync(
                It.IsAny<Guid>(),
                It.IsAny<UserCredential>()))
            .ReturnsAsync(true);

        var result = await UserController.ActivateDeactivate(analystId);
        ...
        UserCommandService
            .Verify(s => s.ActivateOrDeactivateAsync(
                It.IsAny<Guid>(),
    ...
    public async Task ActivateDeactivate_ValidationRoles_ReturnStatusCodeForbidden(string role)
    {
        const int statusForbidden = 403;
        const int statusOk = 200;
